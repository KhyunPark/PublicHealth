...
/*
    ...
    It expects to be provided a province and optionally, minimum bed size and characrters in the hospital name. With this information, it will search the list of hospitals for suitable hospitals.
    ...
    Example:
        INPUT: {"province": "강원도", "minBeds": 200, "name": "주" }
        OUTPUT:
        [
            {
                "name":강원도 영월의료원,
...
*/
    ...
    // FROM HERE, LOOK FOR A DATA WHICH IS REALATED TO INPUT WHICH PEOPLE WROTE FROM THE FILE OF ALL INFORMATION ABOUT HOSPITALS
    ...
    if ($in_prov !== "") {
        ...
        for ($i = 0; $i < count($all_lines_arr); $i++) {
            ...
            $pos_dept_pos = strpos($department, $in_prov);  // Find name in department.
            $pos_name_pos = 0; // Give default position.
            if ($in_name !== "") $pos_name_pos = strpos($hospital_name, $in_name);  // Find name in hospital name.

            if ($pos_dept_pos !== FALSE
                && $num_beds > $in_min_beds
                && $pos_name_pos !== FALSE)
            {
                $hospital_name = $this_hospital[1];
        ...
        if (substr($json_str, -1) === ",") $json_str = substr($json_str, 0, -1);  // Remove the last comma from the last item in the array.
    }
